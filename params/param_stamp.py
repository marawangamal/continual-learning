...
def visdom_name(args):
    ...
    iCaRL = (
        ...
        and checkattr(args, "add_buffer")
        and checkattr(args, "bce")
    ...
    name = "{fb}{replay}{param_reg}{xdg}{icarl}{fromp}{bud}".format(
        ...
        replay="{}{}{}".format(
            args.replay,
            "D" if checkattr(args, "distill") else "",
            (
                "-aGEM"
                if hasattr(args, "use_replay") and args.use_replay == "inequality"
                else ""
            ),
        ),
        param_reg=(
            "-par{}-{}".format(args.reg_strength, args.importance_weighting)
            if checkattr(args, "weight_penalty")
            else ""
        ),
        xdg=(
            ""
            if (not checkattr(args, "xdg")) or args.gating_prop == 0
...
def get_param_stamp_from_args(args, no_boundaries=False):
    ...
    model = define.define_classifier(
        args=args, config=config, device="cpu", depth=depth, stream=no_boundaries
    )
    ...
    if train_gen:
        generator = define.define_vae(
            args=args, config=config, device="cpu", depth=depth
        )
...
def get_param_stamp(
    ...
    multi_n_stamp = (
        "{n}{joint}{cum}-{sce}".format(
            n=args.contexts,
            joint="-Joint" if checkattr(args, "joint") else "",
            cum="-Cummulative" if checkattr(args, "cummulative") else "",
            sce=args.scenario,
        )
        if hasattr(args, "contexts")
        else ""
    )
    stream_stamp = (
        "-{stream}{fuzz}".format(
            stream=args.stream,
            fuzz=(
                "{}-".format(args.fuzziness)
                if args.stream == "fuzzy-boundaries"
                else "-"
            ),
        )
        if no_boundaries
    ...
    problem_stamp = "{exp}{stream}{norm}{aug}{multi_n}".format(
        exp=args.experiment,
        stream=stream_stamp,
        norm="-N" if hasattr(args, "normalize") and args.normalize else "",
    ...
    model_stamp = (
        ...
        else "H{}--{}".format(feature_extractor_name, model_name)
    )
    if verbose:
    ...
    if checkattr(args, "pre_convE") and hasattr(args, "depth") and args.depth > 0:
        ltag = (
            ""
            if ((not hasattr(args, "convE_ltag")) or args.convE_ltag == "none")
            else "-{}{}".format(
        ...
        pre = ""
    freeze_conv = (
        checkattr(args, "freeze_convE") and hasattr(args, "depth") and args.depth > 0
    )
    ...
    train_stamp = "i{num}-lr{lr}-b{bsz}{pre}{freeze}-{optim}{mom}{neg}{recon}".format(
        ...
        mom=(
            "-m{}".format(args.momentum)
            if args.optimizer == "sgd"
            and hasattr(args, "momentum")
            and args.momentum > 0
            else ""
        ),
        neg=(
            "-{}".format(args.neg_samples)
            if (
                args.scenario == "class"
                and (not checkattr(args, "gen_classifier"))
                and (not no_boundaries)
            )
            else ""
        ),
        recon=(
            "-{}".format(args.recon_loss)
            if (
                checkattr(args, "gen_classifier")
                or (hasattr(args, "replay") and args.replay == "generative")
            )
            else ""
        ),
    )
    ...
    if checkattr(args, "weight_penalty") or checkattr(args, "precondition"):
        ...
        if args.importance_weighting == "fisher":
            param_reg_stamp += "-FI{}{}{}{}{}{}".format(
                ...
                (
                    "Emp"
                    if args.fisher_labels == "true"
                    else (
                        "Pred"
                        if args.fisher_labels == "pred"
                        else ("Sam" if args.fisher_labels == "sample" else "All")
                    )
                ),
                (
                    "B{}".format(args.fisher_batch)
                    if (hasattr(args, "fisher_batch") and args.fisher_batch > 1)
                    else ""
                ),
                # -use a separate term per task or a forgetting coefficient:
                (
                    "-offline"
                    if checkattr(args, "offline")
                    else (
                        "-forg{}".format(args.gamma)
                        if hasattr(args, "gamma") and args.gamma < 1
                        else ""
                    )
                ),
            )
        ...
        replay_stamp = "{rep}{KD}{use}{model}{gi}{lrg}".format(
            ...
            use=(
                "-{}{}".format(
                    ...
                    (
                        ""
                        if ((not hasattr(args, "eps_agem")) or args.eps_agem == 0)
                        else args.eps_agem
                    ),
                )
                if hasattr(args, "use_replay") and (not args.use_replay == "normal")
                else ""
            ),
            model=(
                "" if (replay_model_name is None) else "-{}".format(replay_model_name)
            ),
            gi=(
                "-gi{}".format(args.gen_iters)
                if (
                    hasattr(args, "gen_iters")
                    and (replay_model_name is not None)
                    and (not args.iters == args.gen_iters)
                )
                else ""
            ),
            lrg=(
                "-glr{}".format(args.lr_gen)
                if (
                    hasattr(args, "lr_gen")
                    and (replay_model_name is not None)
                    and (not args.lr == args.lr_gen)
                )
                else ""
            ),
        )
    ...
    if use_memory_buffer:
        buffer_opts = "b{bud}{cap}{sel}".format(
            bud=args.budget,
            cap="-FC" if checkattr(args, "use_full_capacity") else "",
            sel=(
                args.sample_selection if hasattr(args, "sample_selection") else "random"
            ),
        )
        use = "{}{}{}".format(
            "addB-" if checkattr(args, "add_buffer") else "",
    ...
    if checkattr(args, "bce"):
        bin_stamp = (
            "--BCE_dist"
            if (checkattr(args, "bce_distill") and args.scenario == "class")
            else "--BCE"
        )
