...
def set_method_options(args, **kwargs):
    ...
    if checkattr(args, "brain_inspired"):
        ...
        args.dg_gates = (
            True  # --> gating based on internal context (has hyper-param 'dg_prop')
        )
...
def set_default_values(
    args, also_hyper_params=True, single_context=False, no_boundaries=False
):
    # -set default-values for certain arguments based on chosen experiment
    args.normalize = (
        args.normalize if args.experiment in ("CIFAR10", "CIFAR100") else False
    )
    args.depth = (
        (
            5
            if (args.experiment in ("CIFAR10", "CIFAR100"))
            or checkattr(args, "reducedResNet")
            else 0
        )
    ...
    args.fc_lay = (
        (1 if checkattr(args, "reducedResNet") else 3)
        if args.fc_lay is None
        else args.fc_lay
    )
    args.channels = (
        (20 if checkattr(args, "reducedResNet") else 16)
    ...
    if not single_context:
        ...
        args.iters = (
            (2000 if args.experiment == "splitMNIST" else 5000)
            if args.iters is None
            else args.iters
        )
    args.lr = (
        (0.001 if args.experiment == "splitMNIST" else 0.0001)
        if args.lr is None
        else args.lr
    )
    args.batch = (
        (128 if args.experiment in ("splitMNIST", "permMNIST") else 256)
    ...
    if checkattr(args, "separate_networks"):
        ...
        args.fc_units = (
            (
                ...
                else (1000 if args.experiment == "permMNIST" else 2000)
            )
            if args.fc_units is None
            else args.fc_units
        )
    if hasattr(args, "fc_units_sep"):
        args.fc_units_sep = (
            (100 if args.experiment == "splitMNIST" else 400)
            if args.fc_units_sep is None
            else args.fc_units_sep
        )
    if hasattr(args, "fc_units_gc"):
        args.fc_units_gc = 85 if args.fc_units_gc is None else args.fc_units_gc
        args.fc_lay_gc = (
            (3 if args.experiment == "splitMNIST" else 2)
            if args.fc_lay_gc is None
            else args.fc_lay_gc
        )
        args.z_dim_gc = (
            (5 if args.experiment == "splitMNIST" else 20)
            if args.z_dim_gc is None
            else args.z_dim_gc
        )
    if hasattr(args, "recon_loss"):
        args.recon_loss = (
            ("MSE" if args.experiment in ("CIFAR10", "CIFAR100") else "BCE")
            if args.recon_loss is None
            else args.recon_loss
        )
    if hasattr(args, "dg_type"):
        args.dg_type = (
            ("context" if args.scenario == "domain" else "class")
            if args.dg_type is None
            else args.dg_type
        )
    if hasattr(args, "budget"):
        args.budget = (
            (10 if args.experiment == "permMNIST" else 100)
        ...
        if hasattr(args, "sample_selection"):
            args.sample_selection = (
                (
                    ...
                    else ("herding" if checkattr(args, "icarl") else "random")
                )
                if args.sample_selection is None
                else args.sample_selection
            )
    ...
    if not single_context:
        args.acc_log = (
            args.iters
            if (not hasattr(args, "acc_log")) or args.acc_log is None
            else args.acc_log
        )
        args.loss_log = (
            args.iters
            if (not hasattr(args, "loss_log")) or args.loss_log is None
            else args.loss_log
        )
        args.sample_log = (
            args.iters
            if (not hasattr(args, "sample_log")) or args.sample_log is None
            else args.sample_log
        )

    # -set default-values for certain arguments based on chosen scenario & experiment
    if (
        hasattr(args, "scenario")
        and args.scenario == "task"
        and hasattr(args, "gating_prop")
    ):
        ...
        args.gating_prop = (
            (
                ...
                else (0.9 if args.experiment == "splitMNIST" else 0.6)
            )
    ...
    if also_hyper_params:
        ...
        if no_boundaries:
            args.si_c = 10.0 if args.si_c is None else args.si_c
        elif args.scenario == "task":
            args.si_c = (
                (
                    10.0
                    if args.experiment == "splitMNIST"
                    else (100.0 if args.experiment == "CIFAR100" else 10.0)
                )
            ...
            args.ewc_lambda = (
                (
                    100000.0
                    if args.experiment == "splitMNIST"
                    else (1000.0 if args.experiment == "CIFAR100" else 100.0)
                )
            ...
            args.si_c = (
                (
                    50000.0
                    if args.experiment == "splitMNIST"
                    else (500.0 if args.experiment == "CIFAR100" else 10.0)
                )
                if args.si_c is None
                else args.si_c
            )
            ...
            args.si_c = (
                (5000.0 if args.experiment == "splitMNIST" else 5.0)
                if args.si_c is None
                else args.si_c
            )
            args.ewc_lambda = (
                (1000000000.0 if args.experiment == "splitMNIST" else 100.0)
                if args.ewc_lambda is None
                else args.ewc_lambda
            )
        if hasattr(args, "reg_strength"):
            args.reg_strength = (
                (
                    args.si_c
                    if checkattr(args, "si")
                    else (args.ewc_lambda if checkattr(args, "ewc") else 1.0)
                )
                if args.reg_strength is None
                else args.reg_strength
            )
            ...
            args.data_size = (
                (
                    ...
                    if args.experiment == "splitMNIST"
                        ...
                        else (5000 if args.experiment == "CIFAR100" else 10000)
                    )
                )
        ...
        if args.scenario == "task" and hasattr(args, "dg_prop"):
            args.dg_prop = (
                (0.0 if args.experiment == "splitMNIST" else 0.0)
                if args.dg_prop is None
                else args.dg_prop
            )
        elif args.scenario == "domain" and hasattr(args, "dg_prop"):
            args.dg_prop = (
                (0.1 if args.experiment == "splitMNIST" else 0.5)
                if args.dg_prop is None
                else args.dg_prop
            )
        elif args.scenario == "class" and hasattr(args, "dg_prop"):
            args.dg_prop = (
                (0.1 if args.experiment == "splitMNIST" else 0.7)
                if args.dg_prop is None
                else args.dg_prop
            )
    if hasattr(args, "tau"):
        ...
        args.tau = (
            (
                (
                    ...
                    else (10.0 if args.scenario == "domain" else 1000.0)
                )
                if args.experiment == "splitMNIST"
                else 1.0
            )
...
def check_for_errors(args, pretrain=False, **kwargs):
    if pretrain:
        if checkattr(args, "augment") and not args.experiment in (
            "CIFAR10",
            "CIFAR100",
        ):
            raise ValueError(
                "Augmentation is only supported for 'CIFAR10' or 'CIFAR-100'."
            )
    if not pretrain:
        if (checkattr(args, "separate_networks") or checkattr(args, "xdg")) and (
            not args.scenario == "task"
        ):
            raise ValueError(
                "'XdG' or 'SeparateNetworks' can only be used with --scenario='task'."
            )
        # -Replay-through-Feedback model is not (yet) implemented with all possible options
        if checkattr(args, "feedback") and (
            checkattr(args, "precondition")
            or (
                hasattr(args, "use_replay")
                and args.use_replay in ("inequality", "both")
            )
        ):
            raise NotImplementedError(
                "Replay-through-Feedback currently does not support gradient projection."
            )
        if checkattr(args, "feedback") and checkattr(args, "xdg"):
            raise NotImplementedError(
                "Replay-through-Feedback currently does not support XdG (in the encoder)."
            )
        if (
            checkattr(args, "feedback")
            and args.importance_weighting == "fisher"
            and checkattr(args, "fisher_kfac")
        ):
            raise NotImplementedError(
                "Replay-through-Feedback currently does not support using KFAC Fisher."
            )
        if checkattr(args, "feedback") and checkattr(args, "bce"):
            raise NotImplementedError(
                "Replay-through-Feedback currently does not support binary classification loss."
            )
        ...
        if (
            hasattr(args, "importance_weighting")
            and args.importance_weighting == "owm"
            and checkattr(args, "weight_penalty")
        ):
            raise NotImplementedError(
                "OWM-based importance weighting not supported with parameter weight penalty."
            )
        if (
            hasattr(args, "importance_weighting")
            and args.importance_weighting == "si"
            and checkattr(args, "precondition")
        ):
            raise NotImplementedError(
                "SI-based importance weighting not supported with parameter pre-conditioning."
            )
        # -FROMP has a limited range of options it can be combined with
        if (
            checkattr(args, "fromp")
        ...
        if (
            checkattr(args, "fromp")
            and hasattr(args, "replay")
            and not args.replay == "none"
        ):
            raise NotImplementedError("FROMP is not supported combined with replay.")
        if checkattr(args, "fromp") and (
            checkattr(args, "weight_penalty") or checkattr(args, "precondition")
        ):
            raise NotImplementedError(
                "FROMP is not supported combined with parameter regularization."
            )
        # -the Generative Classifier implemented here cannot be combined with other approaches
        if (
            checkattr(args, "gen_classifier")
            and hasattr(args, "replay")
            and not args.replay == "none"
        ):
            raise NotImplementedError(
                "The Generative Classifier is not supported with replay."
            )
        if checkattr(args, "gen_classifier") and (
            checkattr(args, "weight_penalty") or checkattr(args, "precondition")
        ):
            raise NotImplementedError(
                "The Generative Classifier is not supported with parameter regularization."
            )
        if checkattr(args, "gen_classifier") and checkattr(args, "fromp"):
            raise NotImplementedError(
                "The Generative Classifier is not supported with FROMP."
            )
        # -a conditional generative model for GR is only supported in combination with Replay-through-Feedback
        if (
            checkattr(args, "per_class") or checkattr(args, "dg_gates")
        ) and not checkattr(args, "feedback"):
            raise NotImplementedError(
                "A VAE with separate mode per class or context-specific gates in the decoder is "
        ...
        trainable_conv = (
            hasattr(args, "depth")
            and args.depth > 0
            and ((not checkattr(args, "freeze_convE")) or checkattr(args, "hidden"))
        )
        if checkattr(args, "xdg") and trainable_conv:
            print(
                "Note that XdG is only applied to the fully connected layers of the network."
            )
        if checkattr(args, "fromp") and trainable_conv:
            print(
                "Note that FROMP is only applied to the fully connected layers of the network."
            )
        if checkattr(args, "fisher_kfac") and trainable_conv:
            print(
                "Note that parameter regularization based on KFAC Fisher is only applied to "
        ...
        if (
            ...
            print(
                "Note that OWM is only applied to the fully connected layers of the network."
            )
