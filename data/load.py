    ...
    if capacity is not None and len(dataset) < capacity:
        dataset = ConcatDataset(
            [
                ...
                for _ in range(int(np.ceil(capacity / len(dataset))))
            ]
        )
...
def get_singlecontext_datasets(
    name, data_dir="./store/datasets", normalize=False, augment=False, verbose=False
):
    ...
    testset = get_dataset(
        name, type="test", dir=data_dir, verbose=verbose, normalize=normalize
    )
...
def get_context_set(
    ...
    if name == "splitMNIST":
        ...
        if train_set_per_class:
            raise NotImplementedError(
                "Permuted MNIST currently has no support for separate training dataset per class"
            )
    elif name == "CIFAR10":
        data_type = "CIFAR10"
    elif name == "CIFAR100":
    ...
    if contexts > config["classes"] and not name == "permMNIST":
        raise ValueError(
            "Experiment '{}' cannot have more than {} contexts!".format(
                name, config["classes"]
            )
        )
    # -how many classes per context?
    classes_per_context = (
        10 if name == "permMNIST" else int(np.floor(config["classes"] / contexts))
    )
    ...
    if name == "permMNIST":
        ...
        testset = get_dataset(
            data_type, type="test", dir=data_dir, target_transform=None, verbose=verbose
        )
        # generate pixel-permutations
        if exception:
            permutations = [None] + [
                np.random.permutation(config["size"] ** 2) for _ in range(contexts - 1)
            ]
        else:
            permutations = [
                np.random.permutation(config["size"] ** 2) for _ in range(contexts)
            ]
        ...
        for context_id, perm in enumerate(permutations):
            target_transform = (
                transforms.Lambda(lambda y, x=context_id: y + x * classes_per_context)
                if scenario in ("task", "class")
                and not (scenario == "task" and singlehead)
                else None
            )
        ...
        perm_class_list = (
            np.array(list(range(classes)))
            if exception
            else np.random.permutation(list(range(classes)))
        )
        ...
        labels_per_dataset_train = (
            ...
            if train_set_per_class
                ...
                list(
                    np.array(range(classes_per_context))
        ...
        labels_per_dataset_test = [
            list(
                np.array(range(classes_per_context)) + classes_per_context * context_id
            )
            for context_id in range(contexts)
        ]
        ...
        for labels in labels_per_dataset_train:
            ...
            train_datasets.append(
                SubDataset(trainset, labels, target_transform=target_transform)
            )
        test_datasets = []
        for labels in labels_per_dataset_test:
            ...
            test_datasets.append(
                SubDataset(testset, labels, target_transform=target_transform)
            )
