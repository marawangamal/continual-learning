...
if __name__ == "__main__":
    ...
    budget_list = (
        budget_list_CIFAR100
        if args.experiment == "CIFAR100"
        else budget_list_splitMNIST
    )
    ...
    for budget in budget_list:
        args.budget = budget
        ER[budget] = {}
        ER[budget] = collect_all(
            ER[budget],
            seed_list,
            args,
            name="Experience Replay - budget = {}".format(budget),
        )
    ...
    for budget in budget_list:
        args.budget = budget
        AGEM[budget] = {}
        AGEM[budget] = collect_all(
            AGEM[budget], seed_list, args, name="A-GEM - budget = {}".format(budget)
        )
    ...
    if not checkattr(args, "no_fromp"):
        ...
        for budget in budget_list:
            if budget <= budget_limit_FROMP:
                ...
                FROMP[budget] = {}
                FROMP[budget] = collect_all(
                    FROMP[budget],
                    seed_list,
                    args,
                    name="FROMP - budget = {}".format(budget),
                )
        args.fromp = False
    ...
    # name for plot
    plot_name = "summaryExactRep-{}{}-{}".format(
    ...
    h_lines = [
        ...
        np.mean([JOINT[seed] for seed in seed_list]),
    ]
    h_errors = (
        [
            np.sqrt(np.var([BASE[seed] for seed in seed_list]) / (len(seed_list) - 1)),
            np.sqrt(np.var([JOINT[seed] for seed in seed_list]) / (len(seed_list) - 1)),
        ]
        if args.n_seeds > 1
        else None
    )
