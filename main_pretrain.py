...
def handle_inputs():
    ...
    set_default_values(
        args, also_hyper_params=False, single_context=True
    )  # -set defaults based on chosen experiment
...
def run(args, verbose=False):
    ...
    (trainset, testset), config = get_singlecontext_datasets(
        ...
        data_dir=args.d_dir,
        verbose=True,
        normalize=utils.checkattr(args, "normalize"),
        augment=utils.checkattr(args, "augment"),
    )
    ...
    cnn = define.define_standard_classifier(
    ...
    eval_cbs = [
        cb._eval_cb(
            log=acc_log, test_datasets=[testset], visdom=visdom, test_size=args.acc_n
        )
    ]
