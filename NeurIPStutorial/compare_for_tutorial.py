...
def handle_inputs():
    ...
    parser = options.define_args(
        filename="compare", description="Compare and plot performance of CL strategies."
    )
...
if __name__ == "__main__":
    ...
    # name for plot
    plot_name = "tutorialplots-{}{}-{}".format(
    ...
    names += [
        ...
        "SI",
        "LwF",
        "FROMP (b={})".format(args.budget),
        "DGR",
        "ER (b={})".format(args.budget),
    ]
    ...
    for id in ids:
        ...
        for line_id in range(len(prec[args.seed][id])):
            ...
            if len(seed_list) > 1:
                new_sem_line.append(
                    1.96 * np.sqrt(np.var(all_entries) / (len(all_entries) - 1))
                )
    ...
    figure = visual_plt.plot_lines(
        ...
        title=title,
        xlabel="# of contexts",
        ylabel="Average accuracy (on contexts so far)",
