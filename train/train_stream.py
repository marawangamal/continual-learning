...
def train_on_stream(model, datastream, iters=2000, loss_cbs=list(), eval_cbs=list()):
    ...
    for batch_id, (x, y, c) in enumerate(datastream, 1):
        ...
        if (
            hasattr(model, "replay_mode")
            and model.replay_mode == "buffer"
        ...
        y_ = (
            y_
            if (hasattr(model, "replay_targets") and model.replay_targets == "hard")
            else None
        )
        scores_ = (
            scores_
            if (hasattr(model, "replay_targets") and model.replay_targets == "soft")
            else None
        )

        # Train the model on this batch
        loss_dict = model.train_a_batch(
            x, y, c, x_=x_, y_=y_, c_=c_, scores_=scores_, rnt=0.5
        )
...
def train_gen_classifier_on_stream(
    model, datastream, iters=2000, loss_cbs=list(), eval_cbs=list()
):
    ...
    for batch_id, (x, y, _) in enumerate(datastream, 1):
        ...
        for class_id in range(model.classes):
            if class_id in y:
                ...
                loss_dict = getattr(model, "vae{}".format(class_id)).train_a_batch(
                    x_to_use
                )
