...
def train_cl(
    ...
    **kwargs,
    ...
    per_context = model.scenario == "task" or (
        model.scenario == "class" and model.neg_samples == "current"
    )
    per_context_singlehead = per_context and (
        model.scenario == "task" and model.singlehead
    )
    ...
    for context, train_dataset in enumerate(train_datasets, 1):
        ...
        if model.scenario == "task":
            if not model.singlehead:
                ...
                active_classes = [
                    list(
                        range(
                            model.classes_per_context * i,
                            model.classes_per_context * (i + 1),
                        )
                    )
        ...
        if (
            not model.label == "SeparateClassifiers"
        ) and model.optim_type == "adam_reset":
        ...
        for batch_index in range(1, iters_to_use + 1):
            ...
            if ReplayStoredData:
                if per_context:
                    up_to_context = (
                        context if baseline == "cummulative" else context - 1
                    )
                    batch_size_replay = (
                        int(np.ceil(batch_size / up_to_context))
                        if (up_to_context > 1)
                        else batch_size
                    )
                    ...
                    for context_id in range(up_to_context):
                        batch_size_to_use = min(
                            batch_size_replay, len(previous_datasets[context_id])
                        )
                        iters_left_previous[context_id] -= 1
                        if iters_left_previous[context_id] == 0:
                            data_loader_previous[context_id] = iter(
                                get_data_loader(
            ...
            if baseline == "cummulative" and per_context:
                ...
                y = (
                    y - model.classes_per_context * (context - 1)
                ...
                binary_distillation = (
                    hasattr(model, "binaryCE")
                    and model.binaryCE
                    and model.binaryCE_distill
                )
                if (
                    binary_distillation
                    and model.scenario in ("class", "all")
                    and (previous_model is not None)
                ):
                    with torch.no_grad():
                        scores = previous_model.classify(x, no_prototypes=True)[
            ...
            #####-----REPLAYED BATCH-----#####
            if (
                not ReplayStoredData
                and not ReplayGeneratedData
                and not ReplayCurrentData
            ):
                ...
                if not per_context:
                    ...
                    if model.replay_targets == "soft":
                        ...
                        if (
                            ...
                            and model.neg_samples == "all-so-far"
                        ):
                            scores_ = scores_[
                                :, : (model.classes_per_context * (context - 1))
                            ]
                    ...
                    for context_id in range(up_to_context):
                        ...
                        if model.replay_targets == "hard":
                            if not per_context_singlehead:
                                ...
                                )  # -> adjust y to 'active range'
                            y_.append(y_temp.to(device))
                        else:
                            y_.append(None)
                    # If required, get target scores (i.e, [scores_])        -- using previous model, with no_grad()
                    if (model.replay_targets == "soft") and (
            ...
            if ReplayGeneratedData:
                conditional_gen = (
                    True
                    if previous_generator.label == "CondVAE"
                    and (
                        (
                            previous_generator.per_class
                            and previous_generator.prior == "GMM"
                        )
                        or checkattr(previous_generator, "dg_gates")
                    )
                    else False
                )
                if conditional_gen and per_context:
                    ...
                    allowed_classes = (
                        ...
                        else list(range(model.classes_per_context * (context - 1)))
                    )
            ...
            if batch_index <= iters:
                ...
                if (
                    ...
                    and model.importance_weighting == "si"
                ):
            ...
            if generator is not None and batch_index <= gen_iters:
                ...
                loss_dict = generator.train_a_batch(x, x_=x_, rnt=1.0 / context)
        ...
        if context < len(train_datasets) and isinstance(model, ContinualLearner):
            ...
            if model.importance_weighting == "fisher" and (
            ...
            if model.importance_weighting == "owm" and (
                model.weight_penalty or model.precondition
            ):
            ...
            if model.importance_weighting == "si" and (
                model.weight_penalty or model.precondition
            ):
        ...
        if checkattr(model, "use_memory_buffer"):
            ...
            new_classes = (
                list(range(model.classes_per_context))
                if (model.scenario == "domain" or per_context_singlehead)
                else list(
                    range(
                        model.classes_per_context * (context - 1),
                        model.classes_per_context * context,
                    )
                )
            )
                        ...
                        for context_id in range(context):
                            previous_datasets.append(
                                MemorySetDataset(
                                    ...
                                    target_transform=(
                                        (
                                            lambda y, x=model.classes_per_context * context_id: y
                                            + x
                                        )
                                        if (not per_context_singlehead)
                                        else (
...
def train_fromp(
    ...
    **kwargs,
    ...
    for context, train_dataset in enumerate(train_datasets, 1):
        ...
        if context > 1:
            model.optimizer.init_context(
                context - 1,
                reset=(model.optim_type == "adam_reset"),
                classes_per_context=model.classes_per_context,
    ...
    **kwargs,
    ...
    for class_id, train_dataset in enumerate(train_datasets):
        ...
        if epochs is not None:
            data_loader = iter(
                get_data_loader(train_dataset, batch_size, cuda=cuda, drop_last=False)
            )
        ...
        for batch_index in range(1, iters + 1):
            ...
            for eval_cb in eval_cbs:
                if eval_cb is not None:
                    eval_cb(
                        model,
                        batch_index + classes_in_current_context * iters,
                        context=context,
                    )
            for sample_cb in sample_cbs:
                if sample_cb is not None:
