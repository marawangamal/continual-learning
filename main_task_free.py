...
def handle_inputs():
    ...
    set_default_values(
        args, also_hyper_params=True, no_boundaries=True
    )  # -set defaults, some based on chosen options
...
def run(args, verbose=False):
    ...
    if args.stream == "academic-setting":
        label_stream = SharpBoundaryStream(
            n_contexts=args.contexts, iters_per_context=args.iters
        )
    ...
    data_stream = DataStream(
        ...
        return_context=(args.scenario == "task"),
        per_batch=(
            True
            if (args.stream == "academic-setting")
            else checkattr(args, "labels_per_batch")
        ),
    )
    ...
    model = define.define_classifier(
        args=args, config=config, device=device, depth=depth, stream=True
    )
    ...
    n_networks = (
        ...
        else (model.classes if checkattr(args, "gen_classifier") else 1)
    )
    ...
    for network_id in range(n_networks):
        ...
        model_to_set.optim_list = [
            {
                "params": filter(lambda p: p.requires_grad, model_to_set.parameters()),
                "lr": args.lr,
            }
        ]
    ...
    if args.train:
        ...
        train_fn = (
            train_gen_classifier_on_stream
            if checkattr(args, "gen_classifier")
            else train_on_stream
        )
        # -perform training
        train_fn(
            model,
            data_stream,
            iters=args.iters * args.contexts,
            eval_cbs=eval_cbs,
            loss_cbs=loss_cbs,
        )
    ...
    for context_id in range(args.contexts):
        acc = evaluate.test_acc(
            model,
            test_datasets[context_id],
            verbose=False,
            context_id=context_id,
            allowed_classes=(
                list(
                    range(
                        config["classes_per_context"] * context_id,
                        config["classes_per_context"] * (context_id + 1),
                    )
                )
