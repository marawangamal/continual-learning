...
class GenerativeClassifier(nn.Module):
    ...
    def sample(self, size, only_x=True, class_id=None, **kwargs):
        ...
        for sample_id in range(size):
            ...
            selected_class_id = (
                np.random.randint(0, self.classes, 1)[0]
                if class_id is None
                else class_id
            )
            ...
            if not only_x:
                y = (
                    torch.cat(
                        [y, torch.LongTensor([selected_class_id]).to(self._device())]
                    )
                    if (sample_id > 0)
    ...
    def classify(self, x, allowed_classes=None, **kwargs):
        ...
        scores = torch.zeros(
            [x.size(0), len(allowed_classes)],
            dtype=torch.float32,
            device=self._device(),
        )
        for class_id in allowed_classes:
            if self.from_latent:
                scores[:, class_id] = getattr(
                    self, "vae{}".format(class_id)
                ).get_latent_lls(x)
            else:
                scores[:, class_id] = getattr(
                    self, "vae{}".format(class_id)
                ).estimate_lls(x, S=self.S, importance=self.importance)
        return scores
