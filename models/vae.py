...
class VAE(ContinualLearner):
    ...
    def get_name(self):
        ...
        return "{}={}{}{}{}".format(
            self.label, convE_label, fcE_label, z_label, decoder_label
        )
    ...
    def loss_function(self, x, x_recon, mu, z, logvar=None, batch_weights=None):
        ...
        batch_size = x.size(0)
        x_recon = (
            (x_recon[0].view(batch_size, -1), x_recon[1].view(batch_size, -1))
            if self.network_output == "split"
            else x_recon.view(batch_size, -1)
        )
        reconL = self.calculate_recon_loss(
            x=x.view(batch_size, -1), average=True, x_recon=x_recon
        )  # -average over pixels
    ...
    def estimate_lls(self, x, S="mean", importance=True):
        ...
        if S == "mean":
            if importance:
                ...
                log_p_z = self.calculate_log_p_z(z_mu)
                # Calculate log_q_z_x
                z_mu_dummy = torch.zeros_like(
                    z_mu
                )  # to avoid unnecessary gradient tracking in next computation
                log_q_z_x = lf.log_Normal_diag(
                    z_mu_dummy, mean=z_mu_dummy, log_var=z_logvar, average=False, dim=1
                )
            else:
            ...
            log_likelihood = (
                (log_p_x_z + log_p_z - log_q_z_x) if importance else log_p_x_z
            )
        else:
            # --> Use [S] importance/Monte Carlo samples
            # Define tensor in which to store the log-likelihoods of each sample
            all_lls = torch.zeros(
                [S, x.size(0)], dtype=torch.float32, device=self._device()
            )
            ...
            for s_id in range(S):
                ...
                x_recon = self.decode(z)
                # -calculate p_x_z (under Gaussian observation model with unit variance)
                log_p_x_z = lf.log_Normal_standard(
                    x=x, mean=x_recon, average=False, dim=-1
                )
    ...
    def train_a_batch(self, x, x_=None, rnt=0.5, **kwargs):
        ...
        if x_ is not None:
            ...
            # Calculate losses
            reconL_r, variatL_r = self.loss_function(
                x=x_, x_recon=recon_batch, mu=mu, z=z, logvar=logvar
            )
