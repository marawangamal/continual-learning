...
def define_classifier(args, config, device, depth=0, stream=False):
    if checkattr(args, "separate_networks"):
        model = define_separate_classifiers(
            args=args, config=config, device=device, depth=depth
        )
    elif checkattr(args, "feedback"):
        model = define_rtf_classifier(
            args=args, config=config, device=device, depth=depth
        )
    elif checkattr(args, "gen_classifier"):
        model = define_generative_classifer(
            args=args, config=config, device=device, depth=depth
        )
    elif stream:
        model = define_stream_classifier(
            args=args, config=config, device=device, depth=depth
        )
    else:
        model = define_standard_classifier(
            args=args, config=config, device=device, depth=depth
        )
    return model
...
def define_stream_classifier(args, config, device, depth=0):
    ...
    model = Classifier(
        ...
        xdg_prob=args.gating_prop if checkattr(args, "xdg") else 0.0,
...
def define_rtf_classifier(args, config, device, depth=0):
    ...
    model = CondVAE(
        ...
        dg_prop=args.dg_prop if hasattr(args, "dg_prop") else 0.0,
...
def init_params(model, args, verbose=False):
    ...
    if checkattr(args, "pre_convE") and hasattr(model, "depth") and model.depth > 0:
        ...
        utils.load_checkpoint(
            model.convE, model_dir=args.m_dir, name=load_name, verbose=verbose
        )
