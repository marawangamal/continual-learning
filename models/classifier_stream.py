...
class Classifier(ContinualLearner, MemoryBuffer):
    ...
    def __init__(
        ...
        if self.xdg_prob > 0.0:
            self.fcE = MLP_gates(
                ...
                gate_size=n_contexts,
                gating_prop=xdg_prob,
                final_gate=True,
                device=device,
            )
        else:
        ...
        # --> classifier
        if self.multihead:
            self.classifier = fc_multihead_layer(
                mlp_output_size,
                classes,
                n_contexts,
                excit_buffer=True,
    ...
    def forward(self, x, context=None):
        ...
        if (
            (self.xdg_prob > 0.0 or self.multihead)
            and (context is not None)
            and (type(context) == np.ndarray or context.dim() < 2)
        ):
            context_one_hot = lf.to_one_hot(
                context, classes=self.n_contexts, device=self._device()
            )

        hidden = self.convE(x)
        flatten_x = self.flatten(hidden)
        final_features = (
            self.fcE(flatten_x, context_one_hot)
            if self.xdg_prob > 0.0
            else self.fcE(flatten_x)
        )
        out = (
            self.classifier(final_features, context_one_hot)
            if self.multihead
            else self.classifier(final_features)
        )
        return out
    ...
    def train_a_batch(
        self, x, y, c=None, x_=None, y_=None, c_=None, scores_=None, rnt=0.5, **kwargs
    ):
        ...
        if x_ is not None:
            ...
            if (y_ is not None) and (y_ is not None):
                if self.binaryCE:
                    binary_targets_ = lf.to_one_hot(y_.cpu(), y_hat.size(1)).to(
                        y_.device
                    )
                    predL_r = (
        ...
        if x_ is None or self.use_replay == "inequality":
            ...
            loss_total = (
                loss_replay if (x is None) else rnt * loss_cur + (1 - rnt) * loss_replay
            )
        ...
        return {
            ...
            "distil_r": (
                distilL_r.item()
                if (scores_ is not None and distilL_r is not None)
                else 0
            ),
            "param_reg": (
