...
class FeatureExtractor(torch.nn.Module):
    ...
    def train_discriminatively(
        self, train_loader, iters, classes, lr=0.001, optimizer="adam"
    ):
        ...
        self.classifier = fc_layer(
            self.conv_out_units, classes, excit_buffer=True, nl="none"
        ).to(self._device())

        # Define optimizer
        optim_list = [
            {"params": filter(lambda p: p.requires_grad, self.parameters()), "lr": lr},
        ]
        self.optimizer = (
            optim.SGD(optim_list)
            if optimizer == "sgd"
            else optim.Adam(optim_list, betas=(0.9, 0.999))
        )
        ...
        while iteration < iters:
            ...
            for batch_idx, (data, y) in enumerate(train_loader):
                ...
                accuracy = (
                    None
                    if y is None
                    else (y == y_hat.max(1)[1]).sum().item() / data.size(0)
                )
