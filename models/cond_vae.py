...
class CondVAE(ContinualLearner):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.gate_size = (
            (contexts if dg_type == "context" else classes) if self.dg_gates else 0
        )
        ...
        self.toZ = fc_layer_split(
            mlp_output_size, z_dim, nl_mean="none", nl_logvar="none"
        )

        ##>----Classifier----<##
        if classifier:
            self.classifier = fc_layer(
                mlp_output_size, classes, excit_buffer=True, nl="none"
            )
        ...
        real_h_dim_down = (
            fc_units
            if fc_layers > 1
            else self.convE.out_units(image_size, ignore_gp=True)
        )
        if self.dg_gates:
        ...
        if self.dg_gates:
            self.fcD = MLP_gates(
                input_size=fc_units,
                output_size=self.convE.out_units(image_size, ignore_gp=True),
                layers=fc_layers - 1,
                hid_size=fc_units,
                drop=fc_drop,
                batch_norm=fc_bn,
    ...
    def get_name(self):
        ...
        fcE_label = (
            "{}--".format(self.fcE.name)
            if self.fc_layers > 1
            else "{}{}-".format("h" if self.depth > 0 else "i", self.conv_out_units)
        )
        z_label = "z{}{}".format(
            self.z_dim,
            (
        ...
        decoder_label = (
            "_{}{}".format("tg" if self.dg_type == "context" else "cg", self.dg_prop)
            if self.dg_gates
            else ""
        )
        return "{}={}{}{}{}{}".format(
            self.label, convE_label, fcE_label, z_label, class_label, decoder_label
        )
    ...
    def decode(self, z, gate_input=None):
        ...
        if (
            self.dg_gates
            and (gate_input is not None)
            and (type(gate_input) == np.ndarray or gate_input.dim() < 2)
        ):
            gate_input = lf.to_one_hot(
                gate_input, classes=self.gate_size, device=self._device()
            )

        # -put inputs through decoder
        hD = self.fromZ(z, gate_input=gate_input) if self.dg_gates else self.fromZ(z)
        image_features = (
            self.fcD(hD, gate_input=gate_input) if self.dg_gates else self.fcD(hD)
        )
        ...
        **kwargs,
        ...
        if self.prior == "GMM":
            if sample_mode is None:
                if (allowed_classes is None and class_probs is None) or (
                    not self.per_class
                ):
                    ...
                    mode_probs = (
                        ...
                        else [p / sum(unweighted_probs) for p in unweighted_probs]
                    )
                    sampled_modes = np.random.choice(
                        allowed_modes, size, p=mode_probs, replace=True
                    )
                    y_used = np.array(
                        [int(mode / self.modes_per_class) for mode in sampled_modes]
                    )
                ...
                y_used = (
                    np.repeat(int(sample_mode / self.modes_per_class), size)
                    if self.per_class
                    else None
                )
        else:
        ...
        if (y_used is None) and (self.dg_gates):
            if allowed_classes is None and class_probs is None:
                ...
                if allowed_classes is None:
                    allowed_classes = [i for i in range(len(class_probs))]
                y_used = np.random.choice(
                    allowed_classes, size, p=class_probs, replace=True
                )
        ...
        if self.dg_gates and self.dg_type == "context":
            if self.scenario == "domain":
                context_used = (
                    np.random.randint(0, self.gate_size, size)
                    if (allowed_domains is None)
                    else np.random.choice(allowed_domains, size, replace=True)
                )
            else:
                classes_per_context = int(self.classes / self.gate_size)
                context_used = np.array(
                    [int(class_id / classes_per_context) for class_id in y_used]
                )
        ...
        with torch.no_grad():
            X = self.decode(
                z,
                gate_input=(
                    (context_used if self.dg_type == "context" else y_used)
                    if self.dg_gates
                    else None
                ),
            )
    ...
    def calculate_log_p_z(self, z, y=None, y_prob=None, allowed_classes=None):
        ...
        if self.prior == "GMM":
            ...
            n_modes = (
                ...
                if (((y is not None) or (y_prob is not None)) and self.per_class)
    ...
    def calculate_variat_loss(
        self, z, mu, logvar, y=None, y_prob=None, allowed_classes=None
    ):
        """Calculate reconstruction loss for each element in the batch.
        ...
        if self.prior == "standard":
            ...
            ## Calculate "log_p_z" (log-likelihood of "reparameterized" [z] based on selected priors)
            log_p_z = self.calculate_log_p_z(
                z, y=y, y_prob=y_prob, allowed_classes=allowed_classes
            )
            # ----->  log_p_z: [batch_size]

            ## Calculate "log_q_z_x" (entropy of "reparameterized" [z] given [x])
            log_q_z_x = lf.log_Normal_diag(
                z, mean=mu, log_var=logvar, average=False, dim=1
            )
    ...
    def loss_function(
        ...
        if logvar is not None:
            actual_y = (
                torch.tensor([allowed_classes[i.item()] for i in y]).to(self._device())
                if ((allowed_classes is not None) and (y is not None))
                else y
            )
            if y is None and scores is not None:
                y_prob = F.softmax(scores / self.KD_temp, dim=1)
                if allowed_classes is not None and len(allowed_classes) > y_prob.size(
                    1
                ):
                    n_batch = y_prob.size(0)
                    zeros_to_add = torch.zeros(
                        n_batch, len(allowed_classes) - y_prob.size(1)
                    )
        ...
        if y is not None and y_hat is not None:
            ...
            predL = lf.weighted_average(
                predL, weights=batch_weights, dim=0
            )  # -> average over batch
        else:
    ...
    def train_a_batch(
        ...
        **kwargs,
        ...
        if x is not None:
            ...
            context_tensor = None
            if self.dg_gates and self.dg_type == "context":
                context_tensor = torch.tensor(np.repeat(context - 1, x.size(0))).to(
                    self._device()
                )
            ...
            recon_batch, y_hat, mu, logvar, z = self(
                x,
                gate_input=(
                    (context_tensor if self.dg_type == "context" else y)
                    if self.dg_gates
                    else None
                ),
                full=True,
            ...
            loss_cur = (
                self.lamda_rcl * reconL
                + self.lamda_vl * variatL
                + self.lamda_pl * predL
            )
        ...
        if x_ is not None:
            ...
            if (not type(x_) == list) and (not (self.dg_gates and PerContext)):
                ...
                if self.dg_gates and self.dg_type == "class":
                    if y_[0] is not None:
                        ...
                        if y_predicted.size(1) < self.classes:
                            ...
                            zeros_to_add = torch.zeros(
                                n_batch, self.classes - y_predicted.size(1)
                            )
                            zeros_to_add = zeros_to_add.to(self._device())
                ...
                x_temp_ = x_
                gate_input = (
                    (contexts_ if self.dg_type == "context" else y_predicted)
                    if self.dg_gates
                    else None
                )
                recon_batch, y_hat_all, mu, logvar, z = self(
                    x_temp_, gate_input=gate_input, full=True
                )
            ...
            for replay_id in range(n_replays):
                ...
                if (type(x_) == list) or (PerContext and self.dg_gates):
                    ...
                    if self.dg_gates and self.dg_type == "class":
                        if y_ is not None and y_[replay_id] is not None:
                            ...
                            y_predicted = y_predicted + replay_id * len(
                                active_classes[0]
                            )
                        else:
                            y_predicted = F.softmax(
                                scores_[replay_id] / self.KD_temp, dim=1
                            )
                            if y_predicted.size(1) < self.classes:
                                # in case of Task-IL, add zeros before and after:
                                n_batch = y_predicted.size(0)
                                zeros_to_add_before = torch.zeros(
                                    n_batch, replay_id * y_predicted.size(1)
                                )
                                zeros_to_add_before = zeros_to_add_before.to(
                                    self._device()
                                )
                                zeros_to_add_after = torch.zeros(
                                    n_batch,
                                    self.classes
                                    - (replay_id + 1) * y_predicted.size(1),
                                )
                                zeros_to_add_after = zeros_to_add_after.to(
                                    self._device()
                                )
                                y_predicted = torch.cat(
                                    [
                                        zeros_to_add_before,
                                        y_predicted,
                                        zeros_to_add_after,
                    ...
                    recon_batch, y_hat_all, mu, logvar, z = self(
                        x_temp_, full=True, gate_input=gate_input
                    )

                # --if needed, remove predictions for classes not active in the replayed context
                y_hat = (
                    y_hat_all
                    if (active_classes is None or y_hat_all is None)
                ...
                (
                    ...
                    x=x_temp_,
                    y=y_[replay_id] if (y_ is not None) else None,
                    x_recon=recon_batch,
                    y_hat=y_hat,
                    scores=scores_[replay_id] if (scores_ is not None) else None,
                    mu=mu,
                    z=z,
                    logvar=logvar,
                    allowed_classes=(
                ...
                # Weigh losses as requested
                loss_replay[replay_id] = (
                    self.lamda_rcl * reconL_r[replay_id]
                    + self.lamda_vl * variatL_r[replay_id]
                )
        ...
        loss_total = (
            loss_replay
            if (x is None)
            else (loss_cur if x_ is None else rnt * loss_cur + (1 - rnt) * loss_replay)
        )
