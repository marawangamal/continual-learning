...
def visualize_images(tensor, title, env, win=None, w=400, h=400, nrow=8):
    ...
    _WINDOW_CASH[win] = env.images(
        tensor, win=_WINDOW_CASH.get(win), nrow=nrow, opts=options
    )
...
def visualize_scalars(scalars, names, title, iteration, env, ylabel=None):
    ...
    scalars = [
        (
            s.cpu().numpy()
            if (hasattr(s, "cpu") and hasattr(s.cpu(), "numpy"))
            else np.array([s])
        )
        for s in scalars
    ]
    num = len(scalars)
    X = (
        np.column_stack(np.array([iteration] * num))
        if (num > 1)
        else np.array([iteration] * num)
    )
