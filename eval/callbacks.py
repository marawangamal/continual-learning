    ...
    def sample_cb(generator, batch, context=1, class_id=None, **kwargs):
        ...
        if batch % log == 0:
            ...
            evaluate.show_samples(
                ...
                visdom=visdom,
                size=sample_size,
                visdom_title="Samples{}".format(
                    " VAE-{}".format(class_id) if class_id is not None else ""
                ),
            )
...
def _eval_cb(
    ...
    def eval_cb(classifier, batch, context=1):
        ...
        iteration = (
            ...
            else (context - 1) * iters_per_context + batch
        )
    ...
    def cb(bar, iter, loss_dict, context=1):
        ...
        if progress_bar and bar is not None:
            context_stm = (
                ""
                if (contexts is None)
                else " Context: {}/{} |".format(context, contexts)
            )
            bar.set_description(
                "<CLASSIFIER> |{t_stm} training loss: {loss:.3} | training accuracy: {prec:.3} |".format(
                    t_stm=context_stm,
        ...
        if (visdom is not None) and (iteration % log == 0):
            ...
            visual_visdom.visualize_scalars(
                ...
                names=names,
                iteration=iteration,
                title="CLASSIFIER: loss ({})".format(visdom["graph"]),
                env=visdom["env"],
...
def _VAE_loss_cb(
    ...
    def cb(bar, iter, loss_dict, context=1):
        ...
        if progress_bar and bar is not None:
            ...
            bar.set_description(
                "  <VAE>      |{t_stm} training loss: {loss:.3} |{acc}".format(
                    t_stm=context_stm,
                    loss=loss_dict["loss_total"],
                    acc=(
                        " training accuracy: {:.3} |".format(loss_dict["accuracy"])
        ...
        if (visdom is not None) and (iteration % log == 0):
            ...
            visual_visdom.visualize_scalars(
                ...
                names=names,
                iteration=iteration,
                title="VAE: loss ({})".format(visdom["graph"]),
                env=visdom["env"],
                ylabel="training loss",
            )
    ...
    def cb(bar, iter, loss_dict, class_id=0):
        ...
        if progress_bar and bar is not None:
            class_stm = (
                ""
                if (classes is None)
                else " Class: {}/{} |".format(class_id + 1, classes)
            )
            model_stm = "  <multiple VAEs>   " if (classes is None) else "  <VAE>      "
            bar.set_description(
                "{m_stm}|{c_stm} training loss: {loss:.3} |".format(
                    m_stm=model_stm, c_stm=class_stm, loss=loss_dict["loss_total"]
                )
            )
            bar.update(1)
        ...
        if (visdom is not None) and (iter % log == 0):
            ...
            visual_visdom.visualize_scalars(
                ...
                title="VAE{}: loss ({})".format(
                    "" if classes is None else "-{}".format(class_id), visdom["graph"]
                ),
                env=visdom["env"],
                ylabel="training loss",
            )
