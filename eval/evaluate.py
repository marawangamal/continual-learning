...
def test_all_so_far(
        ...
        if (current_context is None) or (i + 1 <= current_context):
            ...
            precs.append(
                test_acc(
                    ...
                    no_context_mask=no_context_mask,
                    context_id=i,
                )
            )
        else:
    ...
    if current_context is None:
        current_context = i + 1
    average_precs = (
        sum([precs[context_id] for context_id in range(current_context)])
        / current_context
    )
    ...
    if plotting_dict is not None:
        for i in range(n_contexts):
            plotting_dict["acc per context"]["context {}".format(i + 1)].append(
                precs[i]
            )
    ...
    if visdom is not None:
        visual_visdom.visualize_scalars(
            precs,
            names=names,
            title="accuracy ({})".format(visdom["graph"]),
            iteration=iteration,
            env=visdom["env"],
        ...
        if n_contexts > 1 and summary_graph:
            visual_visdom.visualize_scalars(
                [average_precs],
                names=["ave"],
                title="ave accuracy ({})".format(visdom["graph"]),
                iteration=iteration,
                env=visdom["env"],
                ylabel="test accuracy",
            )
...
def initiate_plotting_dict(n_contexts):
    ...
    plotting_dict["average"] = (
        []
    )  # average accuracy over all contexts so far: Task-IL  -> only classes in context
    #                                            Class-IL -> all classes so far
    plotting_dict["x_iteration"] = []  # total number of iterations so far
    plotting_dict["x_context"] = (
        []
    )  # number of contexts so far (i.e., context on which training just finished)
...
def show_samples(
    ...
    visdom=None,
    size=32,
    pdf_title="Generated images",
    visdom_title="Samples",
):
    ...
    image_tensor = sample.view(
        -1, config["channels"], config["size"], config["size"]
    ).cpu()
    ...
    if pdf is not None:
        visual_plt.plot_images_from_tensor(
            image_tensor, pdf, title=pdf_title, nrow=nrow
        )
    if visdom is not None:
        visual_visdom.visualize_images(
            tensor=image_tensor,
            title="{} ({})".format(visdom_title, visdom["graph"]),
            env=visdom["env"],
            nrow=nrow,
        )
...
def show_reconstruction(
    model, dataset, config, pdf=None, visdom=None, size=32, context=None
):
    ...
    comparison = torch.cat(
        [
            ...
            recon_batch.view(-1, config["channels"], config["size"], config["size"])[
                :size
            ],
        ]
    ).cpu()
    image_tensor = comparison.view(
        -1, config["channels"], config["size"], config["size"]
    )
    # -denormalize images if needed
    if config["normalize"]:
    ...
    if visdom is not None:
        visual_visdom.visualize_images(
            tensor=image_tensor,
            title="Reconstructions ({})".format(visdom["graph"]),
            env=visdom["env"],
            nrow=nrow,
        )
