...
def preprocess(
    feature_extractor, dataset_list, config, batch=128, message="<PREPROCESS>"
):
    ...
    progress_bar.set_description(
        "{} | dataset {}/{} |".format(message, 0, len(dataset_list))
    )
    for dataset_id in range(len(dataset_list)):
        ...
        all_features = torch.empty(
            (len(loader.dataset), config["channels"], config["size"], config["size"])
        )
        ...
        progress_bar.set_description(
            "{} | dataset {}/{} |".format(message, dataset_id + 1, len(dataset_list))
        )
    progress_bar.close()
