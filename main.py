...
def handle_inputs():
    ...
    parser = options.define_args(
        filename="main",
        description="Run an individual continual learning experiment "
    ...
    set_method_options(
        args
    )  # -if a method's "convenience"-option is chosen, select components
    set_default_values(
        args, also_hyper_params=True
    )  # -set defaults, some are based on chosen scenario / experiment
...
def run(args, verbose=False):
    ...
    (train_datasets, test_datasets), config = get_context_set(
        ...
        data_dir=args.d_dir,
        normalize=checkattr(args, "normalize"),
        verbose=verbose,
    ...
    use_feature_extractor = checkattr(args, "hidden") or (
        checkattr(args, "freeze_convE")
        and (not args.replay == "generative")
        and (not checkattr(args, "add_buffer"))
    ...
    if use_feature_extractor and depth > 0:
        ...
        feature_extractor = define.define_feature_extractor(
        ...
        config = (
            config.copy()
        )  # -> make a copy to avoid overwriting info in the original config-file
    ...
    if (feature_extractor is not None) and args.depth > 0:
        ...
        train_datasets = utils.preprocess(
            feature_extractor,
            train_datasets,
            config,
            batch=args.batch,
        ...
        test_datasets = utils.preprocess(
            feature_extractor,
            test_datasets,
            config,
            batch=args.batch,
    ...
    model = define.define_classifier(
        args=args, config=config, device=device, depth=depth
    )

    # Some type of classifiers consist of multiple networks
    n_networks = (
        len(train_datasets)
        if (checkattr(args, "separate_networks") or checkattr(args, "gen_classifier"))
        else 1
    )
    ...
    for network_id in range(n_networks):
        model_to_set = (
            getattr(model, "context{}".format(network_id + 1))
            if checkattr(args, "separate_networks")
            else (
                getattr(model, "vae{}".format(network_id))
        ...
        if not checkattr(args, "fromp"):
            model_to_set.optim_list = [
                {
                    "params": filter(
                        lambda p: p.requires_grad, model_to_set.parameters()
                    ),
                    "lr": args.lr,
                }
            ]
            ...
            if model_to_set.optim_type in ("adam", "adam_reset"):
                model_to_set.optimizer = optim.Adam(
                    model_to_set.optim_list, betas=(0.9, 0.999)
                )
            elif model_to_set.optim_type == "sgd":
                model_to_set.optimizer = optim.SGD(
                    model_to_set.optim_list,
    ...
    if verbose:
        if checkattr(args, "gen_classifier") or checkattr(args, "separate_networks"):
            ...
            utils.print_model_info(
                model.vae0 if checkattr(args, "gen_classifier") else model.context1,
                message=message,
            )
        else:
    ...
    if (
        isinstance(model, ContinualLearner)
        and checkattr(args, "xdg")
        and args.gating_prop > 0.0
    ):
        model.mask_dict = {}
    ...
    use_fisher = (
        ...
        and (checkattr(args, "precondition") or checkattr(args, "weight_penalty"))
    )
    if isinstance(model, ContinualLearner) and use_fisher:
        ...
        model.fisher_labels = (
            args.fisher_labels if hasattr(args, "fisher_labels") else "all"
        )
        ...
        if not model.offline:
            model.gamma = args.gamma if hasattr(args, "gamma") else 1.0
    ...
    if isinstance(model, ContinualLearner) and hasattr(args, "replay"):
        ...
        model.KD_temp = args.temp if hasattr(args, "temp") else 2.0
    ...
    if checkattr(args, "fromp"):
        model.optimizer = fromp_optimizer.opt_fromp(
            model, lr=args.lr, tau=args.tau, betas=(0.9, 0.999)
        )
    ...
    train_gen = (
        True
        if (args.replay == "generative" and not checkattr(args, "feedback"))
        else False
    )
    if train_gen:
        if verbose:
    ...
    if isinstance(model, ContinualLearner) and hasattr(args, "use_replay"):
        ...
        model.eps_agem = args.eps_agem if hasattr(args, "eps_agem") else 0.0
    ...
    use_memory_buffer = (
        checkattr(args, "prototypes")
        or checkattr(args, "add_buffer")
        or args.replay == "buffer"
    ...
    if isinstance(model, MemoryBuffer) and use_memory_buffer:
        ...
        model.sample_selection = (
            args.sample_selection if hasattr(args, "sample_selection") else "random"
        )
    ...
    param_stamp = get_param_stamp(
        ...
        feature_extractor_name=(
            feature_extractor.name if (feature_extractor is not None) else None
        ),
        verbose=verbose,
    )
    ...
    plotting_dict = (
        evaluate.initiate_plotting_dict(args.contexts)
        if (checkattr(args, "pdf") or checkattr(args, "results_dict"))
        else None
    )
    ...
    if utils.checkattr(args, "visdom"):
        ...
        env_name = "{exp}{con}-{sce}".format(
            exp=args.experiment, con=args.contexts, sce=args.scenario
        )
    ...
    generator_loss_cbs = (
        [
            cb._VAE_loss_cb(
                ...
                model=model if checkattr(args, "feedback") else generator,
                contexts=args.contexts,
                iters_per_context=(
                    args.iters if checkattr(args, "feedback") else args.g_iters
                ),
            )
        ]
        if (train_gen or checkattr(args, "feedback"))
        else [None]
    )
    loss_cbs = (
        [
            (
                cb._gen_classifier_loss_cb(
                    ...
                    visdom=visdom if args.loss_log > args.iters else None,
                )
                if checkattr(args, "gen_classifier")
                else cb._classifier_loss_cb(
                    log=args.loss_log,
                    visdom=visdom,
                    model=model,
                    contexts=args.contexts,
        ...
        if (not checkattr(args, "feedback"))
        else generator_loss_cbs
    )
    ...
    sample_cbs = (
        [
            cb._sample_cb(
                ...
                test_datasets=(
                    None if checkattr(args, "gen_classifier") else test_datasets
                ),
            )
        ]
        if (
            train_gen
            or checkattr(args, "feedback")
            or checkattr(args, "gen_classifier")
        )
        and not no_samples
        else [None]
    )
    ...
    eval_cbs = (
        ...
        if (not checkattr(args, "prototypes"))
        and (not checkattr(args, "gen_classifier"))
        else [None]
    )
    ...
    context_cbs = [
        cb._eval_cb(
            ...
            visdom=(
                visdom
                if checkattr(args, "prototypes") or checkattr(args, "gen_classifier")
                else None
            ),
            iters_per_context=args.iters,
            test_size=args.acc_n,
            S=args.eval_s if hasattr(args, "eval_s") else 1,
        )
    ]
    ...
    baseline = (
        "joint"
        if checkattr(args, "joint")
        else ("cummulative" if checkattr(args, "cummulative") else "none")
    )
    ...
    if args.train:
        ...
        train_fn(
            ...
            generator=generator,
            gen_iters=args.g_iters if hasattr(args, "g_iters") else args.iters,
        ...
        utils.load_checkpoint(
            model, args.m_dir, name=load_name, verbose=verbose, strict=False
        )
    ...
    for i in range(args.contexts):
        acc = evaluate.test_acc(
            ...
            allowed_classes=(
                list(
                    range(
                        config["classes_per_context"] * i,
                        config["classes_per_context"] * (i + 1),
                    )
                )
                if (args.scenario == "task" and not checkattr(args, "singlehead"))
                else None
            ),
        )
        if verbose:
    ...
    if verbose:
        print(
            "=> average accuracy over all {} contexts: {:.4f}\n\n".format(
                args.contexts, average_accs
            )
        )
    # -write out to text file
    file_name = "{}/acc-{}{}.txt".format(
        args.r_dir,
        param_stamp,
    ...
    if checkattr(args, "results_dict"):
        file_name = "{}/dict-{}--n{}{}".format(
            args.r_dir,
            param_stamp,
            "All" if args.acc_n is None else args.acc_n,
            "--S{}".format(args.eval_s) if checkattr(args, "gen_classifier") else "",
        )
    ...
    if checkattr(args, "pdf"):
        ...
        if (
            ...
            or args.replay == "generative"
            or checkattr(args, "gen_classifier")
        ):
            evaluate.show_samples(
                (
                    model
                    if checkattr(args, "feedback") or checkattr(args, "gen_classifier")
                    else generator
                ),
                config,
                size=args.sample_n,
            ...
            if not checkattr(args, "gen_classifier"):
                for i in range(args.contexts):
                    evaluate.show_reconstruction(
                        model if checkattr(args, "feedback") else generator,
                        test_datasets[i],
                        config,
                        pdf=pp,
                        context=i + 1,
