...
def handle_inputs():
    ...
    param_reg.add_argument(
        "--offline", action="store_true", help="use Offline EWC rather than Online EWC"
    )
    param_reg.add_argument(
        "--fisher-n-all",
        type=float,
...
def get_result(args):
    ...
    if os.path.isfile(
        "{}/time-{}.txt".format(args.r_dir, param_stamp)
    ) and os.path.isfile("{}/acc-{}.txt".format(args.r_dir, param_stamp)):
...
if __name__ == "__main__":
    ...
    for ewc_lambda in lamda_list:
        args.reg_strength = ewc_lambda
        TRUE128[ewc_lambda] = {}
        TRUE128[ewc_lambda] = collect_all(
            TRUE128[ewc_lambda],
            seed_list,
            args,
    ...
    for ewc_lambda in lamda_list:
        args.reg_strength = ewc_lambda
        ALL500[ewc_lambda] = {}
        ALL500[ewc_lambda] = collect_all(
            ALL500[ewc_lambda],
            seed_list,
            args,
            name="EWC -- FI-labels='all' - n={} (lambda={})".format(
                args.fisher_n_all, ewc_lambda
            ),
        )
    ...
    sem_base = (
        (np.sqrt(np.var(base_entries)) / np.sqrt(args.n_seeds))
        if args.n_seeds > 1
        else None
    )
    ...
    print(
        "---> lambda = {}     --    {}".format(
            ext_lambda_list[np.argmax(mean_all500)], np.max(mean_all500)
        )
    )
    ...
    print(
        "---> lambda = {}     --    {}".format(
            ext_lambda_list[np.argmax(mean_sample)], np.max(mean_sample)
        )
    )
    ...
    print(
        "---> lambda = {}     --    {}".format(
            ext_lambda_list[np.argmax(mean_true)], np.max(mean_true)
        )
    )
    ...
    print(
        "---> lambda = {}     --    {}".format(
            ext_lambda_list[np.argmax(mean_true128)], np.max(mean_true128)
        )
    )
    ...
    # name for plot
    plot_name = "hyperParams-{}{}-{}".format(
        args.experiment, args.contexts, args.scenario
    )
    ...
    errors = (
        [sem_all, sem_all500, sem_sample, sem_true, sem_true128]
        if args.n_seeds > 1
        else None
    )
    names = [
        "All",
        "All - n={}".format(args.fisher_n_all),
        "Sample",
        "True",
        "True - batch=128",
    ]
    ...
    figure = my_plt.plot_lines(
        ...
        colors=colors,
        h_lines=[mean_base],
        h_errors=[sem_base] if args.n_seeds > 1 else None,
        h_labels=["None"],
    )
    ...
    for seed in seed_list:
        ave_prec[seed] = [
            BASE[seed][1],
            ALL[lambda_all][seed][1],
            ALL500[lambda_all500][seed][1],
            SAMPLE[lambda_sample][seed][1],
            TRUE[lambda_true][seed][1],
            TRUE128[lambda_true128][seed][1],
        ]
        train_time[seed] = [
            BASE[seed][0],
            ALL[lambda_all][seed][0],
            ALL500[lambda_all500][seed][0],
            SAMPLE[lambda_sample][seed][0],
            TRUE[lambda_true][seed][0],
            TRUE128[lambda_true128][seed][0],
        ]
    names = [
        "None",
        "All",
    ...
    if len(seed_list) > 1:
        sems = [
            np.sqrt(np.var([ave_prec[seed][id] for seed in seed_list]))
    ...
    if len(seed_list) > 1:
        sems = [
            np.sqrt(np.var([train_time[seed][id] for seed in seed_list]))
            / np.sqrt(len(seed_list))
            for id in ids
        ]
        cis = [
    ...
    for i, name in enumerate(names):
        if len(seed_list) > 1:
            print(
                "{:21s} {:4.0f}  (+/- {:2.0f}),  n={}".format(
                    name, means[i], sems[i], len(seed_list)
                )
            )
        else:
