...
def handle_inputs():
    ...
    parser = options.define_args(
        filename="compare", description="Compare performance of CL strategies."
    )
    ...
    parser.add_argument(
        "--no-context-spec", action="store_true", help="no XdG or Separate Networks"
    )
...
if __name__ == "__main__":
    ...
    ER = {}
    ER = collect_all(
        ER, seed_list, args, name="Experience Replay (budget = {})".format(args.budget)
    )
    args.replay = "none"
    ...
    if not checkattr(args, "no_agem"):
        ...
        AGEM = {}
        AGEM = collect_all(
            AGEM, seed_list, args, name="A-GEM (budget = {})".format(args.budget)
        )
        args.replay = "none"
    ...
    if args.scenario == "class" and not args.neg_samples == "current":
        ...
        ICARL = {}
        ICARL = collect_all(
            ICARL, seed_list, args, name="iCaRL (budget = {})".format(args.budget)
        )
        args.bce = False
    ...
    for seed in seed_list:
        ave_acc[seed] = [
            NONE[seed],
            JOINT[seed],
            0 if EWC is None else EWC[seed],
            0 if SI is None else SI[seed],
            LWF[seed],
            0 if FROMP is None else FROMP[seed],
            DGR[seed],
            0 if BIR is None else BIR[seed],
            ER[seed],
            0 if AGEM is None else AGEM[seed],
        ]
        if args.scenario == "task" and not checkattr(args, "no_context_spec"):
    ...
    if len(seed_list) > 1:
        sems = [
            np.sqrt(
                np.var([ave_acc[seed][id] for seed in seed_list]) / (len(seed_list) - 1)
            )
            for id in ids
        ]
        cis = [
    ...
    figure = visual_plt.plot_bar(
        ...
        names=names,
        colors=colors,
        ylabel="average accuracy (after all contexts)",
        title=title,
